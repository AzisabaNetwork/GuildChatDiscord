...
object ConnectCommand : CommandHandler {
    override suspend fun handle0(interaction: ApplicationCommandInteraction) {
        val defer = interaction.respondPublic { content = ":construction: 現在接続中..." }
        ...
        if (!channel.getEffectivePermissions(interaction.kord.selfId).contains(Permission.ManageWebhooks)) {
            defer.edit { content = "BotがこのチャンネルにWebhookを作成する権限がありません。" }
            return
        }
        val (minecraftUuid) =
            interaction.user.getMinecraftIdName() ?: run {
                defer.edit { content = "Minecraftアカウントが連携されていません。" }
                return
            }
        ...
        if (guild == null || guild.deleted()) {
            defer.edit { content = "ギルド`$guildName`に参加していません。" }
            return
        }
        ...
        val linkedGuildId =
            DatabaseManager.query("SELECT `guild_id` FROM `channels` WHERE `channel_id` = ?") {
        ...
        if (linkedGuildId != -1L) {
            ...
            if (linkedGuild != null && !linkedGuild.deleted()) {
                ...
                defer.edit { content = "`/unconnect`を実行して連携を解除してからもう一度お試しください。" }
            }
            return
        }
        val webhook =
            channel.kord.rest.webhook.createWebhook(channel.id, "ギルドチャット (${guild.name()})") {
                reason = "/connect command from ${interaction.user.tag} (${interaction.user.id})"
            }
        DatabaseManager.query(
            "INSERT INTO `channels` (`guild_id`, `channel_id`, `webhook_id`, `webhook_token`, `created_user_id`) VALUES (?, ?, ?, ?, ?)",
        ) {
        ...
        defer.edit { content = "ギルドチャットを`${guild.name()}`と連携しました。\nメッセージが受信できるようになるまで最大60秒程度かかります。" }
    }
