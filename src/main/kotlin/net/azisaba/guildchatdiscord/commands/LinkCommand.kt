...
object LinkCommand : CommandHandler {
    override suspend fun handle0(interaction: ApplicationCommandInteraction) {
        val defer = interaction.respondEphemeral { content = ":construction: 連携中..." }
        ...
        val uuidName =
            DatabaseManager.query("SELECT `minecraft_uuid`, `minecraft_name` FROM `users` WHERE `link_code` = ?") { stmt ->
        ...
        if (uuidName == null) {
            defer.edit { content = "コードが無効です。" }
            return
        }
        val discordIdAndExpiresAt =
            DatabaseManager.query("SELECT `discord_id`, `expires_at` FROM `users` WHERE `link_code` = ?") { stmt ->
                stmt.setString(1, code)
                stmt.executeQuery().use { rs ->
                    if (rs.next()) {
        ...
        DatabaseManager.getMinecraftUUIDByDiscordId.forget(
            interaction.user.id.value
                .toLong(),
        )
        defer.edit { content = "`$name` (`$uuid`)と連携しました。" }
    }
